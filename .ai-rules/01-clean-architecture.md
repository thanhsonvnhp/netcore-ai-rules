...
> **Root concept**: Xem `docs/Apply Vertical Slice in Clean Architecture .NET API.md` — Clean Architecture cho **biên giới dependency** + Vertical Slice cho **tổ chức feature/use-case** bên trong (đặc biệt Application/Features/...).
...
Domain ← Application ← Infrastructure ← API
   ...
   (xem chi tiết hybrid Vertical Slice + Clean Architecture tại `docs/Apply Vertical Slice in Clean Architecture .NET API.md`)

   `Domain` và `Application` không được import bất kỳ package nào từ Infrastructure (EntityFrameworkCore, Dapper, MassTransit, Polly...).

1. **Đặt Aggregate Root, Value Objects, Domain Events** vào `Domain` layer cho các bounded context cốt lõi (DocumentManagement, Office Management, Organization...).
...
3. **Folder & Namespace Convention (Vertical Slice trong Clean Architecture):**
   ...
   - Ví dụ thực tế (Organization Management module — reference implementation):

    ```text
    SmartOffice.OrganizationManagement.Domain/Aggregates/Products/Product.cs

    SmartOffice.OrganizationManagement.Application/Features/Products/Commands/CreateProduct/CreateProductCommand.cs
    SmartOffice.OrganizationManagement.Application/Features/Products/Commands/CreateProduct/CreateProductCommandHandler.cs
    SmartOffice.OrganizationManagement.Application/Features/Products/Queries/GetProductById/GetProductByIdQueryHandler.cs

    SmartOffice.OrganizationManagement.Infrastructure/Persistence/Configurations/ProductConfiguration.cs
    ```
   ...
   - Xem thêm: `docs/Apply Vertical Slice in Clean Architecture .NET API.md` (sections 7-10: hybrid structure, checklists, worked examples).
...
7. **Mỗi bounded context** (Organization, Document, Identity, Notification...) là một module/project riêng (Domain/Application/Infrastructure/Api), tự quản DbUp scripts riêng trong `.dbup/Scripts/smart_office/<schema>/`.
...
- Hiện tại reference impl (Organization) dùng abstraction `I*DbContext` (ví dụ `IOrganizationDbContext`) được expose từ module Infrastructure, kế thừa `BaseSmartOfficeDbContext` + `ITransactionalDbContext`.
- Handler inject `IOrganizationDbContext` và dùng trực tiếp `DbSet` + `PrepareOutboxEvent` (cho Integration Events).
...
- Outbox, interceptors, read-only context pattern nằm ở **Persistence Building Block** (không lặp trong từng module).

## DON'T

1. **KHÔNG** để controller hoặc Infrastructure class nào kế thừa hoặc inject trực tiếp vào Domain entity.

2. **KHÔNG** để Domain layer tham chiếu `Microsoft.EntityFrameworkCore`, `MassTransit` hoặc bất kỳ framework infrastructure nào.
...
namespace SmartOffice.OrganizationManagement.Domain.Aggregates.Products;

// src/{Module}.Application/Features/{Area}/{UseCase}/
namespace SmartOffice.OrganizationManagement.Application.Features.Products.Commands.CreateProduct;
...
namespace SmartOffice.OrganizationManagement.Infrastructure.Persistence.Configurations;
...
// (module expose IOrganizationDbContext : ITransactionalDbContext từ Infrastructure)
namespace SmartOffice.OrganizationManagement.Application.Abstractions.Data;

public interface IOrganizationDbContext : ITransactionalDbContext
{
...
public void Domain_Should_Not_Reference_Infrastructure()
{
    var result = Types.InAssembly(typeof(Product).Assembly)
        .ShouldNot()
        .HaveDependencyOn("SmartOffice.OrganizationManagement.Infrastructure")
        .GetResult();
    Assert.True(result.IsSuccessful);
}

// Tương tự kiểm tra Application chỉ reference Domain (+ shared BuildingBlocks).
